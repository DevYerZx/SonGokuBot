...
function extractGoogleDriveUrl(text) {
  const match = String(text || "").match(
...
function resolveCommandInput(args, m) {
  const directText = Array.isArray(args) ? args.join(" ").trim() : "";
  if (directText) return directText;

  const quotedMessage =
    m?.quoted || m?.message?.extendedTextMessage?.contextInfo?.quotedMessage || null;

  return extractTextFromMessage(quotedMessage).trim();
}
...
async function downloadApiFile(apiUrl, options) {
  const {
    params,
    outputPath,
    maxBytes,
  ...
  const downloaded = await downloadStreamToFile(response, outputPath, maxBytes, minBytes);
