...
const resolveFileTypeFromBuffer = async (buffer) => {
  if (!buffer || !FileType) return null;
...
exports.generateProfilePicture = async (buffer) => {
  if (!Buffer.isBuffer(buffer)) {
    throw new Error("La imagen de perfil debe ser un buffer valido.");
  }
...
exports.smsg = (client, m, store) => {
  ...
  client.downloadMediaMessage = async (message) => {
    ...
    const messageType = (message.type || mime.split("/")[0]).replace(
      /Message/gi,
      "",
    );
  ...
  if (m.key) {
    ...
    if (!m.isGroup && m.chat.endsWith("@lid")) {
      // FIX Baileys actual: findJidByLid ya no existe
      m.chat = m.chat;
    }
  ...
  if (m.message) {
    ...
    m.msg =
      /viewOnceMessage|viewOnceMessageV2Extension|editedMessage|ephemeralMessage/i.test(
        m.type,
      )
    ...
    m.expiration =
      ...
      client?.messages?.[m.chat]?.array?.slice(-1)[0]?.metadata
        ?.ephemeralDuration ||
      0;
    m.timestamp =
    ...
    if (m.quoted) {
      ...
      if (m.msg?.contextInfo?.participant?.endsWith("@lid"))
        m.msg.contextInfo.participant =
          m?.metadata?.participants?.find(
            (a) => a.lid === m.msg.contextInfo.participant,
          )?.id || m.msg.contextInfo.participant;
      ...
      m.getQuotedObj = async () => {
        ...
        let q = await safeStore.loadMessage(m.chat, m.quoted.id);
        return await exports.smsg(client, q, safeStore);
      };
      m.quoted.key = {
  ...
  m.react = (u) =>
    client.sendMessage(m.chat, { react: { text: u, key: m.key } });
      ...
      try {
        if (/^https?:\/\//.test(content)) {
          ...
          const mime =
            ...
            (await resolveFileTypeFromBuffer(data.data))?.mime ||
            "";
  ...
  client.getName = (jid, withoutContact = false) => {
    ...
    if (id.endsWith("@g.us"))
      ...
      v =
        id === "0@s.whatsapp.net"
          ? { id, name: "WhatsApp" }
          : id === client.decodeJid(client.user?.id || client.user?.jid)
            ? client.user
            : safeStore.contacts[id] || {};
    return (
  ...
  client.getFile = async (PATH, saveToFile = false) => {
    ...
    const type = (await resolveFileTypeFromBuffer(data)) || {
  ...
  client.sendButton = async (
    ...
    const botname = botSettings.namebot || "Lurus";
    ...
    const dynamicButtons = buttons.map((btn) => ({
      ...
      contextInfo: {
        ...
        forwardedNewsletterMessageInfo: {
          newsletterJid: "120363401477412280@newslette",
          serverMessageId: "",
          newsletterName: `Ai Lurus - Nym`,
        },
        externalAdReply: {
          title: botname,
          body: author,
    ...
    if (react) {
      ...
      const anu = await client.query({
        ...
        content: [
          {
            tag: "plaintext",
            attrs: /image|video|audio|sticker|poll/.test(
              Object.keys(media).join("|"),
            )
              ? {
                  mediatype:
                    Object.keys(media).find((key) =>
                      ["image", "video", "audio", "sticker", "poll"].includes(
                        key,
                      ),
                    ) || null,
                }
      ...
      const res =
        ...
        JSON.parse(_query.content[0].content)?.data
      ...
      jid,
      {
        extendedTextMessage: {
          ...
          contextInfo: {
            ...
            isForwarded: true,
            forwardedNewsletterMessageInfo: {
              newsletterJid: "120363401477412280@newsletter",
              serverMessageId: "",
              newsletterName: `Mini Lurus`,
            },
            externalAdReply: {
              title: namebot,
              body: author,
